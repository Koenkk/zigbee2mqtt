...
const thermostat = (min_temp, max_temp, temperature_state_template) => {
    return {
        type: 'climate',
        object_id: 'climate',
        discovery_payload: {
            ...
            temperature_state_template: `{{ value_json.${temperature_state_template} }}`,
            temperature_command_topic: true,
        },
    };
};

// Thermostat Devices which use the current_heating_setpoint topic instead
// occupied_heating_setpoint
const mapCurrentHeatingSetpoint = ['SPZB0001'];

const getTemperatureSetpoint = (model) => {
    if (mapCurrentHeatingSetpoint.includes(model)) {
...
const mapping = {
    ...
    '1TST-EU': [thermostat(7,30,getTemperatureSetpoint('1TST-EU')), cfg.sensor_battery],
    ...
    'SPZB0001': [thermostat(5,30,getTemperatureSetpoint('SPZB0001')), cfg.sensor_battery],
    ...
    'TH1123ZB': [thermostat(7,30,getTemperatureSetpoint('TH1123ZB'))],
    'Zen-01-W': [thermostat(7,30,getTemperatureSetpoint('Zen-01-W'))],
    ...
    discover(entityID, mappedModel, force=false) {
        ...
        mapping[mappedModel.model].forEach((config) => {
            ...
            if (payload.temperature_command_topic) {
                setpoint = getTemperatureSetpoint(mappedModel.model);
                payload.temperature_command_topic = `${stateTopic}/set/${setpoint}`
            }
