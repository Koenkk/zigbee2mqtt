...
const configurations = {
    ...
    'sensor_cover': {
        ...
        discovery_payload: {
            ...
            icon: 'mdi:view-array',
            json_attributes: ['linkquality', 'running', 'last_seen'],
        },
    },
...
class HomeAssistant {
    ...
    discover(ieeeAddr, mappedModel, force=false) {
        ...
        mapping[mappedModel.model].forEach((config) => {
            ...
            const attributeReportInterval = settings.getDevice(ieeeAddr).hasOwnProperty('attribute_report_interval');
            if (settings.get().advanced.hasOwnProperty('availability_interval') || attributeReportInterval) {
                const device = this.zigbee.getDevice(ieeeAddr);
                if (attributeReportInterval || (device && (device.powerSource === 'Mains (single phase)'))) {
                    payload.availability_topic = `${settings.get().mqtt.base_topic}/devices/${friendlyName}/state`;
