...
class HomeAssistant extends Extension {
    ...
    exposeToConfig(exposes, entityType, definition) {
        ...
        if (firstExpose.type === 'light') {
            const colorXY = exposes.find((expose) => expose.features.find((e) => e.name === 'color_xy'));
            const colorHS = exposes.find((expose) => expose.features.find((e) => e.name === 'color_hs'));
            const brightness = exposes.find((expose) => expose.features.find((e) => e.name === 'brightness'));
            const colorTemp = exposes.find((expose) => expose.features.find((e) => e.name === 'color_temp'));
    ...
    getConfigs(resolvedEntity) {
        ...
        let configs;
        if (isDevice) {
            this.mapping[resolvedEntity.definition.model].slice();
        } else { // group
            const exposesByType = {};
            ...
            resolvedEntity.group.members.map((m) => zigbeeHerdsmanConverters.findByDevice(m.getDevice()))
                .filter((m) => m != null).forEach((definition) => {
                    for (const expose of definition.exposes.filter((e) => groupSupportedTypes.includes(e.type))) {
                        let key = expose.type;
                        if (['switch', 'lock', 'cover'].includes(expose.type)) {
