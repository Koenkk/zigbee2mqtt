...
const mapping = {
    ...
    'SV01': [configurations.cover_position, configurations.sensor_temperature, configurations.sensor_pressure, configurations.sensor_battery],
    'Gear': [configurations.cover_position],
};
    ...
    discover(ieeeAddr, mappedModel, force=false) {
        ...
        mapping[mappedModel.model].forEach((config) => {
            ...
            if (payload.position_topic) {
                payload.position_topic = `${settings.get().mqtt.base_topic}/${friendlyName}`;
                payload.set_position_topic = `${settings.get().mqtt.base_topic}/${friendlyName}/set`;
