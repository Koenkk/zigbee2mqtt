...
const thermostat = (minTemp, maxTemp, temperatureStateTemplate) => {
    return {
        type: 'climate',
        object_id: 'climate',
        discovery_payload: {
            ...
            temperature_state_template: `{{ value_json.${temperatureStateTemplate} }}`,
    ...
    discover(entityID, mappedModel, force=false) {
        ...
        mapping[mappedModel.model].forEach((config) => {
            ...
            if (payload.temperature_command_topic) {
                let setpoint = getTemperatureSetpoint(mappedModel.model);
