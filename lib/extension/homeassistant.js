...
const getTemperatureCommandTopicByVendor = (stateTopic, vendor) => {
    if (vendor === 'Eurotronic') {
        topic = `${stateTopic}/set/current_heating_setpoint`;
    }
    else {
        topic = `${stateTopic}/set/occupied_heating_setpoint`;
    }
    return topic;
};
...
const mapping = {
    ...
    '1TST-EU': [thermostat(7,30,'occupied_heating_setpoint'), cfg.sensor_battery],
    ...
    'SPZB0001': [thermostat(5,30,'current_heating_setpoint'), cfg.sensor_battery],
    ...
    'TH1123ZB': [thermostat(7,30,'occupied_heating_setpoint')],
    'Zen-01-W': [thermostat(7,30,'occupied_heating_setpoint')],
    ...
    discover(entityID, mappedModel, force=false) {
        ...
        mapping[mappedModel.model].forEach((config) => {
            ...
            if (payload.temperature_command_topic) {
                payload.temperature_command_topic = getTemperatureCommandTopicByVendor(stateTopic, mappedModel.vendor);
            }
