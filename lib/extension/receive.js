...
class Receive extends Extension {
    ...
    onZigbeeEvent(type, data, resolvedEntity) {
        ...
        const publish = (payload) => {
            ...
            // Check if we have to debounce, device debounce config has higher priority than converter defined value
            const entitySettings = resolvedEntity.settings;
            const meta = resolvedEntity.definition.meta || {};
            const debounceConfig = entitySettings.debounce || meta.debounce;
            const debounceIgnore = entitySettings.debounce_ignore || meta.debounce_ignore;

            // Check if we have to debounce
            if (debounceConfig) {
                this.publishDebounce(
                    data.device.ieeeAddr, payload, debounceConfig,
                    debounceIgnore,
                );
            } else {
