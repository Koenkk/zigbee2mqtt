...
class Receive extends Extension {
    ...
    publishDebounce(ieeeAddr, payload, time, debounceIgnore) {
        console.log('1', ieeeAddr, payload);
        if (!this.debouncers[ieeeAddr]) {
            console.log('2', ieeeAddr);
            this.debouncers[ieeeAddr] = {
                payload: {},
                publish: debounce(() => {
                    console.log('7-1', ieeeAddr, this.debouncers[ieeeAddr].payload);
    ...
    onZigbeeEvent(type, data, resolvedEntity) {
        ...
        const publish = (payload) => {
            ...
            console.log('7-0', data.device.ieeeAddr, resolvedEntity.settings.debounce);
            if (resolvedEntity.settings.debounce) {
                ...
                console.log('7-2', data.device.ieeeAddr, payload);
