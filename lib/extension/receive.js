...
class Receive extends Extension {
    ...
    async onZigbeeStarted() {
        this.coordinator = this.zigbee.getDevicesByTypeLegacy('Coordinator')[0];
    }
    ...
    onZigbeeEvent(type, data, resolvedEntity) {
        ...
        if (type === 'message' && utils.isXiaomiDevice(data.device) && data.device.type === 'Router' && data.groupID) {
            ...
            data.device = this.zigbee.getDeviceByNetworkAddressLegacy(data.groupID);
            resolvedEntity = this.zigbee.resolveEntityLegacy(data.device);
        }
