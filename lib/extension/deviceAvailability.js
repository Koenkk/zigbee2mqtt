...
class DeviceAvailability extends BaseExtension {
    ...
    isPingable(device) {
        ...
        // Device is battery powered and has XXX configure
        const deviceSettings = settings.getDevice(device.ieeeAddr);
        if (utils.isBatteryPowered(device) && deviceSettings.hasOwnProperty('availability_lastseen_timeout')) {
            const lastseen_timeout = (typeof deviceSettings.availability_lastseen_timeout === 'number' && deviceSettings.availability_lastseen_timeout > 0);
            logger.error(`LASTSEEN: '${device.ieeeAddr}' => '${lastseen_timeout}'`);
            return (typeof deviceSettings.availability_lastseen_timeout === 'number' && deviceSettings.availability_lastseen_timeout > 0);
        }
