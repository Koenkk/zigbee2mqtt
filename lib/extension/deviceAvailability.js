...
class DeviceAvailability extends BaseExtension {
    constructor(zigbee, mqtt, state, publishEntityState) {
        ...
        this.availability_lastseen_timeout = settings.get().advanced.availability_lastseen_timeout;
        this.timers = {};
        this.state = {};
    ...
    getAllAllowedDevices() {
        return this.zigbee.getClients().filter((d) => this.isAllowed(d));
    }
    ...
    onMQTTConnected() {
        ...
        this.getAllAllowedDevices().forEach((device) => this.setTimer(device));
    }
    ...
    async handleInterval(device) {
        ...
        if (this.isPingable(device)) {
            ...
            let lastSeenTimeout = this.availability_lastseen_timeout;
            if (deviceSettings && deviceSettings.hasOwnProperty('availability_lastseen_timeout')) {
                lastSeenTimeout = deviceSettings.availability_lastseen_timeout;
            }

            if (lastSeenSecondsAgo <= lastSeenTimeout) {
                this.publishAvailability(device, true);
