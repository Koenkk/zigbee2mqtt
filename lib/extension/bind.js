...
class Bind extends Extension {
    ...
    async onMQTTMessage(topic, message) {
        ...
        if (!source || source.type !== 'device') {
            ...
            if (successfulClusters.length !== 0) {
                await this.setupReporting(source.endpoint, target.type === 'group' ? target.group : target.endpoint,
                    type === 'bind');
            }
        }
    ...
    async groupMembersChanged(group) {
        const bindsToGroup = this.zigbee.getClients().map((c) => c.endpoints).flat()
            .filter((e) => e.binds.find((b) => b.target === group.group));

        for (const bindSource of bindsToGroup) {
            await this.setupReporting(bindSource, group.group, true);
        }
    }
    ...
    async setupReporting(bindSource, bindTarget, enable) {
        const coordinator = this.zigbee.getDevicesByType('Coordinator')[0];
        const coordinatorEndpoint = coordinator.getEndpoint(1);
        const binds = bindSource.binds.filter((b) => b.cluster.name in reportClusters && b.target === bindTarget);
            ...
            for (const target of this.getSetupReportingEndpoints(bind, coordinatorEndpoint)) {
                ...
                try {
                    if (enable) {
                        ...
                        await target.unbind(bind.cluster.name, coordinatorEndpoint);
                    }
                    ...
                    const items = await reportClusters[bind.cluster.name]
                        ...
                        .map((a) => {
                            ...
                            if (!enable) {
                                result.maximumReportInterval = 0xFFFF;
                            }

                            return result;
                        });
                    await target.configureReporting(bind.cluster.name, items);
                    logger.info(`Succesfully ${{true: 'setup', false: 'disabled'}[enable]} reporting for '${entity}' ` +
                        `cluster '${bind.cluster.name}'`);

                    if (!enable) {
                        this.eventBus.emit('reportingDisabled', {device: target.getDevice()});
                    }
                } catch (error) {
                    logger.warn(`Failed to ${{true: 'setup', false: 'disable'}[enable]} reporting for '${entity}' ` +
                        `cluster '${bind.cluster.name}'`);
                }
            }
        }
