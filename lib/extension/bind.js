...
const pollOnMessage = [
    ...
    },
    {
        ...
        // Attributes need to be calculated dynamically depending on the capability
        read: {
            cluster: 'lightingColorCtrl',
            attributes: [],
            attributesForEndpoint: async (endpoint) => {
                const supportedAttrs = await getColorCapabilities(endpoint);
                const readAttrs = ['colorMode'];
                if (supportedAttrs.colorXY) {
                    readAttrs.push("currentX");
                    readAttrs.push('currentY');
                }
        ...
        if (polls.length) {
            ...
            for (const endpoint of toPoll) {
                for (const poll of polls) {
                    ...
                    if (!endpoint.supportsInputCluster(poll.read.cluster)) {
                        continue;
                    }
