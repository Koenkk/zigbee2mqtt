...
class Bind extends Extension {
    ...
    async onZigbeeStarted() {
        setTimeout(() => this.setupReportings(), 1000);
    }
    ...
    shouldSetupReporting(bind, coordinatorEndpoint) {
        const endpoints = bind.target.constructor.name === 'Group' ? bind.target.members : [bind.target];
        const matches = endpoints
            .filter((e) =>
                e.configuredReportings.find((c) => c.cluster.name === bind.cluster.name) &&
                e.binds.find((b) => b.cluster.name == bind.cluster.name && b.target === coordinatorEndpoint));
        return matches.length === 0;
    }

    getSetupReportingTarget(bind) {
        const endpoints = bind.target.constructor.name === 'Group' ? bind.target.members : [bind.target];
        return endpoints.filter((e) => e.supportsInputCluster(bind.cluster.name)).find((e) => true);
    }

    async setupReportings() {
        const coordinator = this.zigbee.getDevicesByType('Coordinator')[0];
        const coordinatorEndpoint = coordinator.getEndpoint(1);
        let didSomething = false;
        for (const client of this.zigbee.getClients()) {
            for (const endpoint of client.endpoints) {
                const binds = endpoint.binds.filter((b) =>
                    b.cluster.name in reportClusters &&
                    this.shouldSetupReporting(b, coordinatorEndpoint) &&
                    (b.target.constructor.name === 'Group' || b.target.getDevice() !== coordinator));
                for (const bind of binds) {
                    didSomething = true;
                    const target = this.getSetupReportingTarget(bind);
                    const entity = `${this.zigbee.resolveEntity(client).name}/${endpoint.ID}`;
                    try {
                        await target.bind(bind.cluster.name, coordinatorEndpoint);
                        const items = await reportClusters[bind.cluster.name].filter(async (a) =>
                            !a.condition || await a.condition(client));
