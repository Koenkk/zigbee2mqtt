...
class Bind extends Extension {
    ...
    async onMQTTMessage(topic, message) {
        ...
        const target = targetKey === 'default_bind_group' ? defaultBindGroup : this.zigbee.resolveEntityLegacy(targetKey);
    ...
    async disableUnnecessaryReportings(target) {
        ...
        for (const endpoint of endpoints) {
            ...
            const requiredClusters = this.zigbee.getClientsLegacy().map((c) => c.endpoints).reduce((a, v) => a.concat(v))
