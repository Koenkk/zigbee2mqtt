...
class Bind extends Extension {
    ...
    async disableUnnecessaryReportings(target) {
        ...
        for (const endpoint of endpoints) {
            ...
            const requiredClusters = this.zigbee.getClientsLegacy().map((c) => c.endpoints)
    ...
    poll(message) {
        ...
        if (polls.length) {
            ...
            const group = message.groupID !== 0 ? this.zigbee.getGroupByIDLegacy(message.groupID) : null;
            if (group) {
