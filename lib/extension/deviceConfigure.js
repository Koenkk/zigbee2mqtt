...
class DeviceConfigure {
    ...
    configure(ieeeAddr, mappedDevice) {
        ...
        if (mappedDevice.configure) {
            ...
            this.queue.push(async (queueCallback) => {
                logger.debug(`Configuring ${friendlyName} (${ieeeAddr}) ...`);

                // Call configure function of this device.
                const coordinator= await this.zigbee.getCoordinator();
                mappedDevice.configure(ieeeAddr, this.zigbee.herdsman, coordinator.endpoints[0], (ok, msg) => {
                    if (ok) {
