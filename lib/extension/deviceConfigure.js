...
class DeviceConfigure {
    ...
    shouldConfigure(device, mappedDevice, settinsDevice, log) {
        if (!device) {
            return false;
        }
        ...
        if (device.interviewing || !device.interviewCompleted) {
            if (log) {
                logger.warn(`Not configuring '${settinsDevice.friendlyName}' because of interview not complete`);
            }
    ...
    async onZigbeeStarted() {
        ...
        for (const device of devices) {
            ...
            if (this.shouldConfigure(device, mappedDevice, settingsDevice)) {
