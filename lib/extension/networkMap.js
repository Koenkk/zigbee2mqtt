...
class NetworkMap extends Extension {
    ...
    async networkScan(includeRoutes) {
        ...
        const devices = this.zigbee.getDevicesLegacy().filter((d) => d.type !== 'GreenPower');
        const lqis = new Map();
        ...
        lqis.forEach((lqi, device) => {
            for (const neighbor of lqi.neighbors) {
                ...
                const neighborDevice = this.zigbee.getDeviceByNetworkAddressLegacy(neighbor.networkAddress);
