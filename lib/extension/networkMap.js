...
class NetworkMap {
    ...
    graphviz(topology) {
        ...
        topology.nodes.forEach((device) => {
            ...
            labels.push(
                `${device.networkAddress} (${device.ieeeAddr})` +
                device.failed.length ? `failed: ${device.failed.join(',')}` : ''
            );
    ...
    async networkScan(includeRoutes) {
        ...
        const devices = (await this.zigbee.getDevices({})).filter((d) => d.type != 'EndDevice');
        const lqis = new Map();
        ...
        for (const device of devices) {
            ...
            if (includeRoutes) {
                try {
                    ...
                    failed[device].push('routingTable');
        ...
        for (const device of devices) {
            ...
            networkMap.nodes.push({
                ...
                modelID: device.modelID, failed: failed[device],
            });
        }
        ...
        lqis.forEach((lqi, device) => {
            for (const neighbor of lqi.neighbors) {
                ...
                routingTables.forEach((device, routingTable) => {
                    if (device.networkAddress === neighbor.networkAddress) {
                        link.routes = routingTable
                            .map((r) => r.table)
                            .filter((t) => t.status === 0 && t.nextHop === device.networkAddress);
                    }
                });
