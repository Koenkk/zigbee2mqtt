...
class NetworkMap {
    ...
    graphviz(zigbee, topology) {
        ...
        topology.nodes.forEach((device) => {
            ...
            labels.push(`0x${device.networkAddress.toString(16)} ${scanNote}`);
            ...
            if (mappedModel) {
                ...
                const zigbeeModel = [device.manufacturerName, device.modelID].filter((a) => a).join(' ');
             ...
             */
            topology.links.filter((e) => (e.sourceIeeeAddr === device.ieeeAddr) || (e.SourceNwkAddr === device.networkAddress))
                .forEach((e) => {
