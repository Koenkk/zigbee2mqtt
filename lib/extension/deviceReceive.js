...
class DeviceReceive {
    ...
    onZigbeeMessage(message, device, mappedDevice) {
        ...
        const publish = (payload) => {
            ...
            case 'ISO_8601':
                payload.last_seen = new Date(last_seen).toISOString();
                break;
            case 'epoch':
            ...
            if (settings.get().advanced.elapsed) {
                if (this.elapsed[device.ieeeAddr]) {
                    payload.elapsed = payload.last_seen - this.elapsed[device.ieeeAddr];
                }
                this.elapsed[device.ieeeAddr] = payload.last_seen;
            }
