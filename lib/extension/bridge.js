...
class Bridge extends Extension {
    ...
    async onMQTTConnected() {
        ...
        this.eventBus.on(`deviceBiningsChanged`, () => this.publishDevices());
            ...
            for (const endpoint of device.endpoints) {
                for (const bind of endpoint.binds) {
                    ...
                    if (bind.target.constructor.name === 'Endpoint') {
                        ...
                        targetKey = ['endpoint', bind.target.getDevice().ieeeAddr, bind.target.ID].join('/');
                    } else {
                        target = {type: 'group', id: bind.target.groupID};
                        targetKey = ['group', bind.target.groupID].join('/');
                    }
                    const source = {
                        endpoint: endpoint.ID,
                    };
                    if (bindings[targetKey]) {
                        bindings[targetKey].clusters.push(bind.cluster.name);
                    } else {
                        bindings[targetKey] = {clusters: [bind.cluster.name], target, source};
                    }
                }
                const data = {
                    clusters: {
