...
class Bridge extends Extension {
    ...
    async bridgeOptions(message) {
        ...
        const diffUpdated = updatedDiff(settings.get(), message);
        const diffAdded = addedDiff(settings.get(), message);
        const diff = objectAssignDeep.noMutate(diffUpdated, diffAdded);
        delete diff.transaction; // TODO
        const restartRequired = settings.apply(diff);
        if (restartRequired) this.restartRequired = true;
        logger.info('Succesfully changed options');
        this.publishInfo();

        // apply homeassistant, permit_join, log_level, last_seen, elapsed
