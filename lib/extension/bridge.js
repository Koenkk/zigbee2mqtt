...
class Bridge extends Extension {
    ...
    async publishDevices() {
        const devices = this.zigbee.getDevices().map((device) => {
            ...
            if (definition) {
                const setConverters = new Set();
                const getConverters = new Set();
                if (definition.toZigbee) {
                    definition.toZigbee.forEach((toZigbee) => {
                        console.log(toZigbee);
                        if (toZigbee.key) {
                            toZigbee.key.forEach((key) => {
                                if (toZigbee.convertSet) {
                                    setConverters.add(key);
                                }
                                if (toZigbee.convertGet) {
                                    getConverters.add(key);
                                }
                            });
                        }
                    });
                ...
                definitionPayload = {
                    ...
                    set_converters: Array.from(setConverters),
                    get_converters: Array.from(getConverters),
                };
            }
