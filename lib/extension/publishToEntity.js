...
class PublishToEntity {
    ...
    async onMQTTMessage(topic, message) {
        ...
        if (entitySettings.type === 'device') {
            const device = await this.zigbee.getDevice({ieeeAddr: entitySettings.entity.ID});
            ...
            if (mappedDevice.hasOwnProperty('endpoint')) {
                const endpoints = mappedDevice.endpoints(device);
            ...
            options = {...options, ...entitySettings.entity};
        } else if (entitySettings.type === 'group') {
            converters = groupConverters;
            entity = this.zigbee.getGroup({groupID: entitySettings.entity.ID});
        }

        if (!entity) {
            logger.error(`Failed to find entity: '${entitySettings.entity.ID}'`);
            this.mqtt.log('entity_not_found', entitySettings.entity.ID);
            return;
        }
        ...
        for (const [key, value] of entries) {
            ...
            try {
                if (topic.type === 'set' && converter && converter.convertSet) {
                    logger.debug(`Publishing set '${topic.type}' '${key}' to '${entitySettings.entity.friendlyName}'`);
                    ...
                    if (result && result.hasOwnProperty('state')) {
                        ...
                        this.publishEntityState(entitySettings.entity.ID, msg);
                    }
                    ...
                    if (
                        ...
                        entitySettings.entity.retrieve_state
                    ) {
                    ...
                    logger.debug(`Publishing get '${topic.type}' '${key}' to '${entitySettings.entity.friendlyName}'`);
                ...
                const message =
                    `Publish '${topic.type}' '${key}' to '${entitySettings.entity.friendlyName}' failed: '${error}'`;
                logger.error(message);
                this.mqtt.log('zigbee_publish_error', message, {entity: entitySettings.entity.ID});
            }
...
module.exports = PublishToEntity;
