...
class BridgeConfig extends BaseExtension {
    ...
    async permitJoin(topic, message) {
        const permit = message.toLowerCase() === 'false' ? false : true;
        let device = null;
        if (!['true', 'false'].includes(message.toLowerCase())) {
            device = this.zigbee.resolveEntity(message);
            assert(device !== null, `Device '${message}' does not exist`);
            device = device.device;
        }

        await this.zigbee.permitJoin(permit, device);
        this.publish();
    }
