...
class BridgeConfig {
    ...
    whitelist(topic, message) {
        ...
        try {
            ...
            settings.whitelistDevice(entity.entity.ID);
            logger.info(`Whitelisted '${entity.entity.friendlyName}'`);
            this.mqtt.log('device_whitelisted', entity.entity.friendlyName);
        } catch (error) {
    ...
    deviceOptions(topic, message) {
        ...
        const entity = settings.getEntity(json.friendly_name).entity;
    ...
    async permitJoin(topic, message) {
        await this.zigbee.permitJoin(message.toString().toLowerCase() === 'true');
        this.publish();
    }
    ...
    async removeOrBan(ban, message) {
        message = message.toString();
        const entity = settings.getEntity(message).entity;
        ...
        const cleanup = () => {
            ...
            logger.info(`Successfully ${ban ? 'banned' : 'removed'} ${entity.friendlyName}`);
        ...
        if (device) {
            try {
                logger.info(`Removing '${entity.friendlyName}'`);
                await device.removeFromNetwork();
                cleanup();
            } catch (error) {
                logger.error(`Failed to ${ban ? 'ban' : 'remove'} ${entity.friendlyName} (${error})`);
            }
        } else {
            cleanup();
        }
