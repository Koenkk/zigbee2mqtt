import type {IncomingMessage, Server, ServerResponse} from "node:http";
import {createServer} from "node:http";
import * as client from "prom-client";
import {type MetricEvent, MetricType, metrics as zhMetrics} from "zigbee-herdsman";
import type Device from "../model/device";
import logger from "../util/logger";
import * as settings from "../util/settings";
import utils, {getZigbee2MQTTVersion} from "../util/utils";
import Extension from "./extension";

export class PrometheusExporter extends Extension {
    #server: Server | undefined;
    #registry!: client.Registry;

    // Counters
    #deviceMessagesReceived!: client.Counter;
    #deviceMessagesFailed!: client.Counter;
    #mqttPublished!: client.Counter;
    #mqttReceived!: client.Counter;
    #deviceJoins!: client.Counter;
    #deviceLeaves!: client.Counter;
    #deviceAnnounces!: client.Counter;
    #deviceNetworkAddressChanges!: client.Counter;
    #deviceMessagesSent!: client.Counter;

    // Gauges
    #buildInfo!: client.Gauge;
    #deviceLinkQuality!: client.Gauge;
    #deviceInfo!: client.Gauge;
    // The next four gauges are populated by their scrape-time collect() callback, so they are never read after assignment.
    // biome-ignore lint/correctness/noUnusedPrivateClassMembers: value produced by collect()
    #deviceLastSeen!: client.Gauge;
    // biome-ignore lint/correctness/noUnusedPrivateClassMembers: value produced by collect()
    #deviceAvailability!: client.Gauge;
    // biome-ignore lint/correctness/noUnusedPrivateClassMembers: value produced by collect()
    #mqttConnected!: client.Gauge;
    // biome-ignore lint/correctness/noUnusedPrivateClassMembers: value produced by collect()
    #permitJoin!: client.Gauge;
    #coordinatorInfo!: client.Gauge;

    // Adapter metrics
    #adapterSendDuration!: client.Histogram;
    #adapterRetries!: client.Counter;
    #requestQueueLength!: client.Gauge;
    #adapterReceiveZclPayload!: client.Counter;
    #adapterReceiveZdoResponse!: client.Counter;

    // Herdsman metric event handler: a single discriminated `metric` event carries all metric types.
    readonly #onMetric = (event: MetricEvent): void => {
        switch (event.type) {
            case MetricType.AdapterSendZclUnicast:
                this.#adapterSendDuration.observe({type: "zcl_unicast", status: event.status}, event.durationSeconds);
                this.#deviceMessagesSent.inc({ieee_address: event.ieeeAddr});
                break;
            case MetricType.AdapterSendZdo:
                this.#adapterSendDuration.observe({type: "zdo", status: event.status}, event.durationSeconds);
                this.#deviceMessagesSent.inc({ieee_address: event.ieeeAddr});
                break;
            case MetricType.AdapterSendZclGroup:
                this.#adapterSendDuration.observe({type: "zcl_group", status: event.status}, event.durationSeconds);
                break;
            case MetricType.AdapterSendZclBroadcast:
                this.#adapterSendDuration.observe({type: "zcl_broadcast", status: event.status}, event.durationSeconds);
                break;
            case MetricType.AdapterRetry:
                this.#adapterRetries.inc({adapter_type: event.adapterType, reason: event.reason});
                break;
            case MetricType.AdapterReceiveZclPayload:
                this.#adapterReceiveZclPayload.inc({cluster_id: String(event.clusterID), was_broadcast: String(event.wasBroadcast)});
                break;
            case MetricType.AdapterReceiveZdoResponse:
                this.#adapterReceiveZdoResponse.inc({cluster_id: String(event.clusterId)});
                break;
            case MetricType.RequestQueueLength:
                this.#requestQueueLength.set({ieee_address: event.ieeeAddr, endpoint_id: String(event.endpointId)}, event.length);
                break;
        }
    };

    override async start(): Promise<void> {
        await super.start();

        this.#registry = new client.Registry();
        client.collectDefaultMetrics({register: this.#registry});

        this.#buildInfo = new client.Gauge({
            name: "zigbee2mqtt_build_info",
            help: "Build information; value is always 1",
            labelNames: ["version", "commit_hash"],
            registers: [this.#registry],
        });

        const {version, commitHash} = await getZigbee2MQTTVersion();
        this.#buildInfo.set({version, commit_hash: commitHash ?? /* v8 ignore next */ "unknown"}, 1);

        this.#deviceMessagesReceived = new client.Counter({
            name: "zigbee2mqtt_device_messages_received_total",
            help: "Total number of Zigbee messages received from a device",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        this.#deviceMessagesFailed = new client.Counter({
            name: "zigbee2mqtt_device_messages_failed_total",
            help: "Total number of Zigbee messages that failed processing",
            labelNames: ["ieee_address", "reason"],
            registers: [this.#registry],
        });

        this.#mqttPublished = new client.Counter({
            name: "zigbee2mqtt_mqtt_messages_published_total",
            help: "Total number of MQTT messages published",
            registers: [this.#registry],
        });

        this.#mqttReceived = new client.Counter({
            name: "zigbee2mqtt_mqtt_messages_received_total",
            help: "Total number of MQTT messages received",
            registers: [this.#registry],
        });

        this.#deviceJoins = new client.Counter({
            name: "zigbee2mqtt_device_joins_total",
            help: "Total number of device join events",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        this.#deviceLeaves = new client.Counter({
            name: "zigbee2mqtt_device_leaves_total",
            help: "Total number of device leave events",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        this.#deviceAnnounces = new client.Counter({
            name: "zigbee2mqtt_device_announces_total",
            help: "Total number of device announce events",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        this.#deviceNetworkAddressChanges = new client.Counter({
            name: "zigbee2mqtt_device_network_address_changes_total",
            help: "Total number of device network address changes",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        this.#deviceLinkQuality = new client.Gauge({
            name: "zigbee2mqtt_device_link_quality",
            help: "Last known link quality indicator (LQI) for a device (0-255)",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        this.#deviceInfo = new client.Gauge({
            name: "zigbee2mqtt_device_info",
            help: "Static device metadata; value is always 1",
            labelNames: ["ieee_address", "friendly_name", "model_id", "vendor", "type", "power_source"],
            registers: [this.#registry],
        });

        this.#adapterSendDuration = new client.Histogram({
            name: "zigbee2mqtt_adapter_send_duration_seconds",
            help: "Duration of adapter send operations in seconds, by type and status",
            labelNames: ["type", "status"],
            buckets: [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 2, 5],
            registers: [this.#registry],
        });

        this.#adapterRetries = new client.Counter({
            name: "zigbee2mqtt_adapter_retries_total",
            help: "Total number of adapter send retries",
            labelNames: ["adapter_type", "reason"],
            registers: [this.#registry],
        });

        this.#requestQueueLength = new client.Gauge({
            name: "zigbee2mqtt_request_queue_length",
            help: "Current length of the per-device request queue",
            labelNames: ["ieee_address", "endpoint_id"],
            registers: [this.#registry],
        });

        this.#adapterReceiveZclPayload = new client.Counter({
            name: "zigbee2mqtt_adapter_receive_zcl_payload_total",
            help: "Total number of ZCL payloads received by the adapter",
            labelNames: ["cluster_id", "was_broadcast"],
            registers: [this.#registry],
        });

        this.#adapterReceiveZdoResponse = new client.Counter({
            name: "zigbee2mqtt_adapter_receive_zdo_response_total",
            help: "Total number of ZDO responses received by the adapter",
            labelNames: ["cluster_id"],
            registers: [this.#registry],
        });

        this.#deviceMessagesSent = new client.Counter({
            name: "zigbee2mqtt_device_messages_sent_total",
            help: "Total number of Zigbee messages sent to a device (ZCL unicast and ZDO)",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
        });

        // Scrape-time gauges reflecting current state; `this` inside collect() is the gauge, so capture the exporter.
        const exporter = this;

        this.#deviceLastSeen = new client.Gauge({
            name: "zigbee2mqtt_device_last_seen_timestamp_seconds",
            help: "Unix timestamp (seconds) a device was last seen",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
            collect() {
                this.reset();
                for (const device of exporter.zigbee.devicesIterator(utils.deviceNotCoordinator)) {
                    if (device.zh.lastSeen != null) {
                        this.set({ieee_address: device.ieeeAddr}, device.zh.lastSeen / 1000);
                    }
                }
            },
        });

        this.#deviceAvailability = new client.Gauge({
            name: "zigbee2mqtt_device_availability",
            help: "Whether a device is currently considered available (1) or not (0), based on last seen and the configured timeout",
            labelNames: ["ieee_address"],
            registers: [this.#registry],
            collect() {
                this.reset();
                for (const device of exporter.zigbee.devicesIterator(utils.deviceNotCoordinator)) {
                    this.set({ieee_address: device.ieeeAddr}, exporter.#isAvailable(device) ? 1 : 0);
                }
            },
        });

        this.#mqttConnected = new client.Gauge({
            name: "zigbee2mqtt_mqtt_connected",
            help: "Whether the MQTT client is currently connected to the broker (1) or not (0)",
            registers: [this.#registry],
            collect() {
                this.set(exporter.mqtt.isConnected() ? 1 : 0);
            },
        });

        this.#permitJoin = new client.Gauge({
            name: "zigbee2mqtt_permit_join",
            help: "Whether joining new devices is currently permitted (1) or not (0)",
            registers: [this.#registry],
            collect() {
                this.set(exporter.zigbee.getPermitJoin() ? 1 : 0);
            },
        });

        this.#coordinatorInfo = new client.Gauge({
            name: "zigbee2mqtt_coordinator_info",
            help: "Static coordinator and network metadata; value is always 1",
            labelNames: ["channel", "pan_id", "extended_pan_id", "coordinator_type", "revision"],
            registers: [this.#registry],
        });

        const coordinatorVersion = await this.zigbee.getCoordinatorVersion();
        const networkParameters = await this.zigbee.getNetworkParameters();
        this.#coordinatorInfo.set(
            {
                channel: String(networkParameters.channel),
                pan_id: String(networkParameters.panID),
                extended_pan_id: String(networkParameters.extendedPanID),
                coordinator_type: coordinatorVersion.type,
                revision: String(coordinatorVersion.meta.revision ?? ""),
            },
            1,
        );

        // Pre-populate device_info for all known devices
        for (const device of this.zigbee.devicesIterator()) {
            this.#setDeviceInfo(device.ieeeAddr, device.name, device.zh.modelID, device.definition?.vendor, device.zh.type, device.zh.powerSource);
        }

        // Subscribe to events
        this.eventBus.onDeviceMessage(this, (data) => {
            const ieeeAddr = data.device.ieeeAddr;

            this.#deviceMessagesReceived.inc({ieee_address: ieeeAddr});
            this.#deviceLinkQuality.set({ieee_address: ieeeAddr}, data.linkquality);
        });

        this.eventBus.onDeviceMessageFailed(this, (data) => {
            this.#deviceMessagesFailed.inc({
                ieee_address: data.device.ieeeAddr,
                reason: data.reason,
            });
        });

        this.eventBus.onMQTTMessagePublished(this, () => {
            this.#mqttPublished.inc();
        });

        this.eventBus.onMQTTMessage(this, () => {
            this.#mqttReceived.inc();
        });

        this.eventBus.onDeviceJoined(this, (data) => {
            const ieeeAddr = data.device.ieeeAddr;
            const friendlyName = data.device.name;
            this.#deviceJoins.inc({ieee_address: ieeeAddr});
            this.#setDeviceInfo(
                ieeeAddr,
                friendlyName,
                data.device.zh.modelID,
                data.device.definition?.vendor,
                data.device.zh.type,
                data.device.zh.powerSource,
            );
        });

        this.eventBus.onDeviceLeave(this, (data) => {
            this.#deviceLeaves.inc({ieee_address: data.ieeeAddr});
        });

        this.eventBus.onDeviceAnnounce(this, (data) => {
            this.#deviceAnnounces.inc({ieee_address: data.device.ieeeAddr});
        });

        this.eventBus.onDeviceNetworkAddressChanged(this, (data) => {
            this.#deviceNetworkAddressChanges.inc({ieee_address: data.device.ieeeAddr});
        });

        this.eventBus.onEntityRemoved(this, (data) => {
            if (!data.entity.isDevice()) return;
            this.#removeDeviceMetrics(data.entity.ieeeAddr, data.name, data.entity);
        });

        zhMetrics.on("metric", this.#onMetric);

        // Start HTTP server
        const {port, host} = settings.get().prometheus_exporter;
        this.#server = createServer(this.#onRequest.bind(this));

        if (host) {
            this.#server.listen(port, host);
            logger.info(`Prometheus exporter listening on ${host}:${port}`);
        } else {
            this.#server.listen(port);
            logger.info(`Prometheus exporter listening on port ${port}`);
        }
    }

    override async stop(): Promise<void> {
        zhMetrics.off("metric", this.#onMetric);
        await new Promise((resolve) => (this.#server ? this.#server?.close(resolve) : resolve(undefined)));
        await super.stop();
    }

    #isAvailable(device: Device): boolean {
        const availabilityOption = typeof device.options.availability === "object" ? device.options.availability : undefined;
        const timeoutMinutes =
            availabilityOption?.timeout ??
            (this.#isActiveDevice(device) ? settings.get().availability.active.timeout : settings.get().availability.passive.timeout);
        const lastSeen = device.zh.lastSeen ?? 0;
        return Date.now() - lastSeen < utils.minutes(timeoutMinutes);
    }

    #isActiveDevice(device: Device): boolean {
        return (
            (device.zh.type === "Router" && device.zh.powerSource !== "Battery") ||
            (device.zh.powerSource !== undefined && device.zh.powerSource !== "Unknown" && device.zh.powerSource !== "Battery")
        );
    }

    #removeDeviceMetrics(ieeeAddr: string, friendlyName: string, entity: Device): void {
        const base = {ieee_address: ieeeAddr};
        this.#deviceMessagesReceived.remove(base);
        this.#deviceMessagesFailed.remove({...base, reason: "no_converter"});
        this.#deviceMessagesFailed.remove({...base, reason: "converter_error"});
        this.#deviceMessagesSent.remove(base);
        this.#deviceJoins.remove(base);
        this.#deviceLeaves.remove(base);
        this.#deviceAnnounces.remove(base);
        this.#deviceNetworkAddressChanges.remove(base);
        this.#deviceLinkQuality.remove(base);
        this.#deviceInfo.remove({
            ...base,
            friendly_name: friendlyName,
            model_id: entity.zh.modelID ?? /* v8 ignore next */ "",
            vendor: entity.definition?.vendor ?? /* v8 ignore next */ "",
            type: entity.zh.type,
            power_source: entity.zh.powerSource ?? /* v8 ignore next */ "",
        });
    }

    #setDeviceInfo(
        ieeeAddr: string,
        friendlyName: string,
        modelId: string | undefined,
        vendor: string | undefined,
        type: string,
        powerSource: string | undefined,
    ): void {
        this.#deviceInfo.set(
            {
                ieee_address: ieeeAddr,
                friendly_name: friendlyName,
                model_id: modelId ?? "",
                vendor: vendor ?? "",
                type,
                power_source: powerSource ?? "",
            },
            1,
        );
    }

    async #onRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
        const url = req.url ?? /* v8 ignore next */ "/";

        if (url === "/metrics") {
            const metrics = await this.#registry.metrics();
            res.setHeader("Content-Type", this.#registry.contentType);
            res.end(metrics);
        } else {
            res.writeHead(200);
            res.end("zigbee2mqtt prometheus exporter");
        }
    }
}
