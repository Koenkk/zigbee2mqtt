...
class Availability extends Extension {
    ...
    async onMQTTMessage(topic, message) {
        ...
        if (match && (!this.zigbee.resolveEntityLegacy(match[1]) || this.zigbee.resolveEntityLegacy(match[1]).name !== match[1])) {
    ...
    async stop() {
        ...
        this.zigbee.getClientsLegacy().forEach((device) => this.publishAvailability(device, false));
    }
