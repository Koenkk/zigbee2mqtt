...
class Report extends Extension {
    ...
    async setupReporting(resolvedEntity) {
        ...
        if (this.settingUp.has(device.ieeeAddr) || this.failed.has(device.ieeeAddr)) return;
        this.settingUp.add(device.ieeeAddr);
        ...
        device.save();
        this.settingUp.delete(device.ieeeAddr);
    }
