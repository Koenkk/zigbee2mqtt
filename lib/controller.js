...
class Controller {
    ...
    handleMQTTMessageConfig(topic, message) {
        ...
        if (option === 'permit_join') {
            ...
            const invalid = `Invalid remove message format expected: ` +
                `{"friendly_name": "FRIENDLY_NAME", ban: boolean} got ${message.toString()}`;
            ...
            const IDByFriendlyName = settings.getIDByFriendlyName(json.friendly_name);
            const deviceID = IDByFriendlyName ? IDByFriendlyName : json.friendly_name;
            ...
            const cleanup = () => {
                ...
                if (settings.removeDevice(deviceID)) {
                    ...
                    this.mqtt.log('device_removed', json.friendly_name);
                } else {
            ...
            if (device) {
                const ban = json.hasOwnProperty('ban') ? json.ban : false;
