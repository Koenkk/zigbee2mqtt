    ...
    handleMQTTMessageConfig(topic, message) {
        ...
        if (option === 'permit_join') {
            ...
            this.mqtt.log('devices', devices);
             ...
             const invalid = `Invalid remove message format expected {friendlyName: 'friendly_name', reJoin: 'True or False'} ` +
            ...
            // Validate message
            if (!json.friendlyName || !json.reJoin) {
                logger.error(invalid);
                return;
            }
            const IDByFriendlyName = settings.getIDByFriendlyName(json.friendlyName);
            const deviceID = IDByFriendlyName ? IDByFriendlyName : json.friendlyName;
            ...
            if (settings.removeDevice(deviceID)) {
                logger.info(`Successfully removed ${deviceID}`);
                this.mqtt.log('device_removed', json.friendlyName);
            } else {
            ...
            if (device) {
                const reJoinParam = json.reJoin ? json.reJoin : 'true';
                var cfg = { reJoin: reJoinParam };
                this.zigbee.removeDevice(deviceID,cfg,(error) => {
                    if (!error) {
                        cleanup();
                    } else {
                        logger.error(`Failed to remove ${deviceID}`);
