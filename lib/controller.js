...
class Controller {
    ...
    async onZigbeeEvent(type, data) {
        ...
        if (type === 'message') {
            ...
            this.mqtt.log('device_connected', {friendly_name: friendlyName});
            ...
            if (data.status === 'successful') {
                ...
                if (entity.mapped) {
                    ...
                    this.mqtt.log('pairing', 'interview_successful', {friendly_name: friendlyName, supported: false});
                }
            } else if (data.status === 'failed') {
                logger.error(`Failed to interview '${friendlyName}', device has not succesfully been paired`);
                this.mqtt.log('pairing', 'interview_failed', {friendly_name: friendlyName});
            } else {
                /* istanbul ignore else */
                if (data.status === 'started') {
                    logger.info(`Starting interview of '${friendlyName}'`);
                    this.mqtt.log('pairing', 'interview_started', {friendly_name: friendlyName});
                }
            }
            ...
            if (type === 'deviceLeave') {
                ...
                this.mqtt.log('device_removed', 'left_network', {friendly_name: friendlyName});
            }
        }
