...
class Controller {
    ...
    handleZigbeeMessage(message) {
        ...
        // Request reports
        if (mappedModel.report) {
            const device = message.endpoints[0].device;
            while (mappedModel.report.length) {
                const rep = mappedModel.report.pop();
                const dev = this.zigbee.shepherd.find(device.ieeeAddr, rep.ep);
                dev.report(rep.cid, rep.attr, rep.minInt, rep.maxInt, rep.repChange, (err) =>
                    logger.error(`configuring report '${rep.cid}' '${rep.attr}': '${err}'`));
            }
        }
