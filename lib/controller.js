...
class Controller {
    ...
    handleZigbeeMessage(message) {
        ...
        const converters = mappedModel.parsers.filter((c) =>
            zigbee2mqtt.hasOwnProperty(c) && zigbee2mqtt[c].cid === cid && zigbee2mqtt[c].type === message.type
        );
        ...
        converters.forEach((converter) => {
            ...
            const payload = zigbee2mqtt[converter].convert(mappedModel, message, publish, settings.getDevice(device.ieeeAddr));
