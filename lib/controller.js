...
class Controller {
    ...
    start() {
        this.zigbee.start(this.handleZigbeeMessage, (error) => {
            if (error) {
                ...
                const subscriptions = [`${settings.get().mqtt.base_topic}/+/set`, `${settings.get().mqtt.base_topic}/bridge/config/+`];
    ...
    handleMQTTMessage(topic, message) {
        ...
        } else if (topic.match(mqttDeviceRegex)) {
            this.handleMQTTMessageDevice(topic, message);
        } else {
    ...
    handleMQTTMessageDevice(topic, message) {
        ...
        Object.keys(json).forEach((key) => {
            ...
            const callback = (error) => {
                ...
                if (!error && key === 'state') {
                    this.mqttPublishDeviceState(deviceID, {state: json[key]})
                }
            };
