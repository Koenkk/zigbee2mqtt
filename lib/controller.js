...
class Controller {
    ...
    publishEntityState(entityID, payload) {
        ...
        if (settings.get().experimental.output === 'json') {
            ...
            const iteratePayload = (message, prefix) => {
                prefix = prefix || '';
                Object.keys(message).forEach((key) => {
                    if (typeof message[key] === 'object' && !Array.isArray(message[key])) {
                        iteratePayload(message[key], prefix+key+'-');
                    } else {
                        this.mqtt.publish(`${entity.friendlyName}/${prefix}${key}`,
