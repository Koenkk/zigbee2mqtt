...
class Controller {
    ...
    async start() {
        ...
        const devices = await this.zigbee.getAllClients();
         ...
         * MQTT start
         */
        await this.mqtt.connect(this.onMQTTMessage, this.onMQTTConnected);
    ...
    async publishEntityState(entityID, payload) {
        ...
        if (entity.type === 'device' && appSettings.mqtt.include_device_information) {
            messagePayload.device = await this.getDeviceInfoForMqtt(entityID);
        }
    ...
    async getDeviceInfoForMqtt(ieeeAddr) {
        const device = await this.zigbee.getDevice(ieeeAddr);
        const {
            type, nwkAddr, manufacturerID, manufName, powerSource, modelId, hwVersion, swBuildId, status, dateCode,
        } = device;
