...
function loadModuleFromText(moduleCode, baseDir) {
    const appDir = baseDir || path.dirname(require.main.filename);
    const moduleFakePath = path.join(appDir, 'external.js');
    const sandbox = {
        require,
        module: {},
        __filename: moduleFakePath,
    };
    vm.runInNewContext(moduleCode, sandbox, moduleFakePath);
    return sandbox.module.exports;
}
function loadModuleFromFile(modulePath, baseDir) {
    const converterCode = fs.readFileSync(modulePath, {encoding: 'utf8'});
    return loadModuleFromText(converterCode, baseDir);
}
