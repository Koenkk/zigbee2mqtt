...
function getEndpoint(device, endpointName) {
    const mappedDevice = zigbeeHerdsmanConverters.findByZigbeeModel(device.modelID);
    let endpoint = null;

    if (mappedDevice && mappedDevice.hasOwnProperty('endpoint')) {
        const endpoints = mappedDevice.endpoint(device);
        const endpointID = endpoints.hasOwnProperty(endpointName) ? endpoints[endpointName] : null;
        if (endpointID) {
    ...
    if (!endpoint) {
        endpoint = device.getEndpoints()[0];
    }
