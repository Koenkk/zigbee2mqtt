...
function init(): void {
    ...
    if (output.includes('syslog')) {
        ...
        const options: KeyValue = {
            ...
            format: winston.format.printf(/* istanbul ignore next */(info) => {
                return `${info.service}: ${info.message}`;
            }),
...
/** @deprecated TODO: change all code references to `warning` */
function warn(message: string, service: string = 'zigbee2mqtt'): void {
    // @ts-expect-error syslog level
    logger.warning(message, {service});
}

function warning(message: string, service: string = 'zigbee2mqtt'): void {
    // @ts-expect-error syslog level
    logger.warning(message, {service});
}

function info(message: string, service: string = 'zigbee2mqtt'): void {
    logger.info(message, {service});
}

function debug(message: string, service: string = 'zigbee2mqtt'): void {
    logger.debug(message, {service});
}

function error(message: string, service: string = 'zigbee2mqtt'): void {
...
function child(options: object): ChildLogger {
    // @ts-expect-error syslog level
...
export default {
    init, logOutput, warn, warning, error, info, debug, setLevel, getLevel, cleanup, addTransport, end, child,
