...
if (output.includes('file')) {
    fx.mkdirSync(current);
    fs.rmdir(current, function(err, result) { if(err) console.log('error', err); });
    fx.mkdirSync(directory);
    fs.symlink(actual, current, function(err, result) { if(err) console.log('error', err); });
}
