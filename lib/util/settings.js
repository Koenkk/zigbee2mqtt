...
function apply(newSettings) {
    const errors = !ajvSetting(newSettings) ?
        ajvSetting.errors.filter((e) => e.keyword !== 'required') : [];
    if (errors.length) {
    ...
    const errorsRestartRequired = !ajvRestartRequired(newSettings) ? ajvRestartRequired.errors : [];
    const restartRequired = !!errorsRestartRequired.find((e) => e.keyword === 'requiresRestart');
