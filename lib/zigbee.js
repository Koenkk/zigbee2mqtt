...
class Zigbee extends events.EventEmitter {
    ...
    async permitJoin(permit, device) {
        let message = '';
        if (permit) {
            if (device) {
                const name = this.resolveEntity(device).name;
                message = `Allowing new devices to join via '${name}'`;
            } else {
                message = 'Allowing new devices to join';
            }
        } else {
            message = 'Disabling joining new devices';
        }

        logger.info(message);
        await this.herdsman.permitJoin(permit, device);
    }
