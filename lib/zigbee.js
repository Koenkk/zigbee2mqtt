...
class Zigbee {
    ...
    handleReady() {
        ...
        const ID = '0x00124b000c034c83'
        const device = devices.find((d) => d.ieeeAddr === ID);
        device.epList.forEach((ep) => {
            const dev = this.shepherd.find(ID, ep);
			dev.report('seMetering', 'instantaneousDemand', 3, 10, 1, (err) => console.log(err));
        });
