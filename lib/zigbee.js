    ...
    async resolveEntity(key) {
        ...
        if (typeof key === 'string') {
            ...
            if (entity.type === 'device') {
                ...
                const name = settings ? settings.friendlyName : device.ieeeAddr;
                return {type: 'device', device, settings: entity, mapped, endpoint, name};
            } else {
                const group = await this.getGroup({groupID: entity.ID});
                return {type: 'group', group, settings: entity, name: group.friendlyName};
            }
        } else {
