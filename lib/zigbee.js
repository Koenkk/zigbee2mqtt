...
class Zigbee {
    ...
    //setup reporting.
    //attributes is either one attribute object containing
    //attr, min, max and change properties
    //or an array of attribute objects.
    //if attributes is a single attribute object, this method will first
    //bind the cluster and then configure reporting for the attribute
    //if attributes is an array of attribute objects,
    //this method will bind the cluster before configuring the
    //first attribute for reporting. For subsequent attributes,
    //the cluster binding is skipped.
    report(ep, cluster, attributes) {
        if(Array.isArray(attributes)) {
            if(attributes.length>0) {
                this.reportAttribute(ep,cluster,attributes[0], true);
                for(var i=1; i<attributes.length; i++) {
                    this.reportAttribute(ep,cluster,attributes[i], false);
                }
            }
        } else {
            //attributes must be a single attribute object
            this.reportAttribute(ep,cluster,attributes, true);
        }
    }

    reportAttribute(ep, cluster, attribute, doBind) {
        console.log("reportAttribute"+cluster);
        ...
        const log=`for ${ep.device.ieeeAddr} - ${cluster} - ${attribute.attr}`;
        ...
        this.queue.push((queueCallback) => {
            ...
            if(doBind) {
                ep.bind(cluster, this.getCoordinator(), (error) => {
                    if(error) {
                        logger.error(`Failed to ibind for reporting ${log} - (${error})`);
                    } else {
