...
class Zigbee extends events.EventEmitter {
    ...
    async getEntityByIDOrFriendlyName(IDorName) {
        const entity = settings.getEntity(IDorName);
        if (entity.type === 'device') {
            return this.getDevice({ieeeAddr: entity.ID});
        } else if (entity.type === 'group') {
            return this.getGroup({groupID: entity.ID});
        } else {
            throw new Error(`Entity '${IDorName}' does not exist`);
        }
    }
