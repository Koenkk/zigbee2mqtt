...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        ...
        const options = {
            will: {
                ...
                payload: 'offline',
                retain: settings.get().advanced.force_disable_retain ? false : true,
            },
        };
    ...
    subscribe(topic) {
        if (!settings.get().advanced.subscribe_to_whole_prefix) {
    ...
    async publish(topic, payload, options, base=settings.get().mqtt.base_topic, skipLog=false, skipReceive=true) {
        ...
        if (settings.get().advanced.force_disable_retain) {
