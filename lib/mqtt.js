...
class MQTT {
    ...
    connect(messageHandler, callback) {
        ...
        if (mqttSettings.key && mqttSettings.cert && mqttSettings.ca) {
            logger.debug(`Using SSL/TLS settings for connection:`);
            logger.debug(` - key path = ${mqttSettings.key}`);
            logger.debug(` - cert path = ${mqttSettings.cert}`);
            logger.debug(` - CA cert path = ${mqttSettings.ca}`);
            options.key = mqttSettings.key;
            options.cert = mqttSettings.cert;
            options.ca = mqttSettings.ca;
        }
