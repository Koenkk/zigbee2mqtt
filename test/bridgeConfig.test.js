...
describe('Bridge config', () => {
    ...
    it('Should allow to get devices', async () => {
        ...
        expect(payload[1]).toStrictEqual({"friendly_name": "bulb", "ieeeAddr": "0x000b57fffec6a5b2", "manufacturerID": 4476, "model": "LED1545G12", "modelID": "TRADFRI bulb E27 WS opal 980lm", "networkAddress": 40369, "powerSource": "Mains (single phase)", "type": "Router"});
    });
    ...
    it('Should allow to get groups', async () => {
        ...
        const payload = JSON.parse(MQTT.publish.mock.calls[0][1]);
        expect(payload).toStrictEqual({"message": [{"ID": 1, "friendly_name": "group_1", "retain": false, 'devices': []}, {"ID": 2, "friendly_name": "group_2", "retain": false, "devices": []}], "type": "groups"});
    });
