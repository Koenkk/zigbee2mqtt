...
describe('User extensions', () => {
    ...
    it('Load user extension', async () => {
        ...
        expect(MQTT.publish).toHaveBeenCalledWith('zigbee2mqtt/extensions/list', stringify(['exampleExtension.js']), { retain: true, qos: 0 }, expect.any(Function));
        MQTT.publish.mockClear();
        MQTT.events.message('zigbee2mqtt/extension/request/read', stringify({"name": "exampleExtension.js"}));
        await flushPromises();
    ...
    it('Load user extension from api call', async () => {
        ...
        await flushPromises();
        MQTT.publish.mockClear();
        MQTT.events.message('zigbee2mqtt/extension/request/save', stringify({"name": "foo.js", "content": extensionCode}));
        await flushPromises();
        expect(MQTT.publish).toHaveBeenCalledWith('zigbee2mqtt/extensions/list', stringify(['foo.js']), { retain: true, qos: 0 }, expect.any(Function));
