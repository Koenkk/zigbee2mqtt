...
If this fails, the `modelID` has to be set manually in `data/database.db`. First find out the `modelID` of your devices from the (Supported devices page)[../information/supported_devices.md]. Then open `data/database.db` and add the `modelId` as higlighted in **bold** below.
