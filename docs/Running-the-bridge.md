*Before you start make sure you have a CC2531 USB sniffer flashed with CC2531ZNP-Prod.hex, if not first go to [Getting started](https://github.com/Koenkk/zigbee2mqtt/wiki/Getting-started).*

In case you want to run zigbee2mqtt in a Docker container see [Docker](https://github.com/Koenkk/zigbee2mqtt/wiki/Docker).

In case you want to run zigbee2mqtt together with Home Assistant via hassio, see the [hassio-zigbee2mqtt add-on](https://github.com/danielwelch/hassio-zigbee2mqtt).
...
*NOTE: [it is reported that Raspbian Jessie doesn't work](https://github.com/Koenkk/zigbee2mqtt/issues/67#issuecomment-392574987).*
...
As an alternative, the device can also be mapped by an ID. This can be handy if you have multiple serial devices connected to your pi. In the example below the device location is: `/dev/serial/by-id/usb-Texas_Instruments_TI_CC2531_USB_CDC___0X00124B0018ED3DDF-if00`
```bash
...
# For Pi Zero use https://raw.githubusercontent.com/sdesalas/node-pi-zero/master/install-node-v8.10.0.sh
...
## 7. Done!
Continue with [Pairing devices](https://github.com/Koenkk/zigbee2mqtt/wiki/Pairing-devices)